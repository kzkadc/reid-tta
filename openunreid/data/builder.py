...
def build_test_dataloader(cfg, one_gpu=False, **kwargs):
    ...
    # build transformer
    if "CORRUPTION" in cfg.TEST:
        test_query_transformers = [
            build_test_transformer(cfg, (c, f) if c is not None else None)
            for c, f in zip(cfg.TEST.CORRUPTION.types, cfg.TEST.CORRUPTION.factors)
        ]
    else:
        test_query_transformers = [build_test_transformer(cfg) for _ in dataset_names]

    test_gallery_transformer = build_test_transformer(cfg)

    # build individual datasets
    datasets, queries, galleries = [], [], []
    for dn, qt in zip(dataset_names, test_query_transformers):
        query_data = build_dataset(
            dn, data_root, "query", del_labels=False, transform=qt
        )
        gallery_data = build_dataset(
            dn, data_root, "gallery", del_labels=False,
            transform=test_gallery_transformer
        )
